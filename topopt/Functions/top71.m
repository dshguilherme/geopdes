...
function [f, xPhys, loop] = top71(nelx,nely,volfrac,penal,rmin,ft)
%% MATERIAL PROPERTIES
E0 = 1;
Emin = 1e-9;
nu = 0.3;
...
F = U;
F(2:2*(nely+1):2+ceil(nelx/20)*2*(nely+1)) = 1;
F = F/sum(F);
...
while change > 0.01
  ...
  fixeddofsA = [2*(nelx+1-ceil(nelx/20))*(nely+1): ...
    2*(nely+1):2*(nelx+1)*(nely+1)];
  K(fixeddofsA,fixeddofsA) = K(fixeddofsA,fixeddofsA) + ...
      1e3*max(diag(K))* ...
      ones(length(fixeddofsA),length(fixeddofsA));
